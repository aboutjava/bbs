...
public class MyBatisGenerator {
    ...
    public void generate(ProgressCallback callback, Set<String> contextIds,
        ...
        //获取表名
        for (Context context : contextsToRun) {
            java.sql.Connection conn = ConnectionFactory.getInstance().getConnection(context.getJdbcConnectionConfiguration());
            CommentProvider.getInstance().initializeMysqlTableName(conn);
            conn.close();
        }
...
//                jdbcConnectionConfiguration);
    ...
    private void writeFile(File file, String content) throws IOException {
        OutputStreamWriter osw = new OutputStreamWriter(new FileOutputStream(file), Charset.forName("utf8"));
        osw.write(content);
        osw.close();
    }
